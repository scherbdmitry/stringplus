...
char *sprintf_eEfgG(spec_struct *options, va_list ap) {
  ...
  if (g_flag) {
    while (digit_fraction) {
      unsigned long long tem = digit_fraction % 10;
      if (tem) {
        break;
      } else {
        digit_fraction /= 10;
  ...
  for (; format[i] != '\0' && stop_scan == 0; i++) {
    ...
    if (format[i] == '%' && stop_scan == 0) {
      ...
      if (s21_strchr("diouxXp", options->specificator) != S21_NULL) {
        ...
        int flag_successes = 0;
        if (options->specificator == 'd' ||
            options->specificator == 'u') {
          flag_successes = sscanf_d(&result, s, &j, options->width);
        }
        if (options->specificator == 'x' ||
            options->specificator == 'X') {
          flag_successes = sscanf_x(&result, s, &j, options->width);
          if (!flag_successes) {
            if (s[j] == '0') {
              result = 0;
              j++;
        ...
        if (options->specificator == 'o') {
          flag_successes = sscanf_o(&result, s, &j, options->width);
        }
        if (options->specificator == 'i') {
          flag_successes = sscanf_i(&result, s, &j, options->width);
        }
        if (options->specificator == 'p') {
          flag_successes = sscanf_x(&result, s, &j, options->width);
          if (!flag_successes) {
            flag_successes = sscanf_hex(&result, s, &j, options->width);
          }
        }
        if (flag_successes) {
          if (options->flag_star_width == 0) {
